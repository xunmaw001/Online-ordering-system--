  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
	    ...
	    <download-excel v-if="isAuth('dingdanxinxi','导出')" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "订单信息.xls">
		      <!-- 导出excel -->
            	<el-button
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','取消') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="quxiaodingdanCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'取消':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','取消') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="quxiaodingdanCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'取消':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','取消') && contents.tableBtnIcon == 0" type="success" size="mini" @click="quxiaodingdanCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'取消':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','配送') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="dingdanpeisongCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'配送':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','配送') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="dingdanpeisongCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'配送':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','配送') && contents.tableBtnIcon == 0" type="success" size="mini" @click="dingdanpeisongCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'配送':'' }}</el-button>
...
//导出excel
      ...
      "菜品分类": "caipinfenlei",    //常规字段
      "图片": "tupian",    //常规字段
      "菜品价格": "caipinjiage",    //常规字段
      "数量": "shuliang",    //常规字段
      "总金额": "zongjine",    //常规字段
      "备注": "beizhu",    //常规字段
      "送餐地址": "songcandizhi",    //常规字段
  ...
  methods: {
    ...
    dingdanpeisongCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
    ...
    chartDialog() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: "group/dingdanxinxi/zhuangtai",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
            let res = data.data;
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          tupian: row.tupian,
          caipinjiage: row.caipinjiage,
          shuliang: row.shuliang,
          zongjine: row.zongjine,
          beizhu: row.beizhu,
          songcandizhi: row.songcandizhi,
